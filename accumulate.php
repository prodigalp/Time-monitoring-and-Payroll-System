<?php
	...
	if(isset($_POST['btnSnd'])) {
		if("Search"==$_POST['btnSnd']) {
			...
			//$q1 = "SELECT * FROM tblbackup WHERE empnum='$emp' && date>=concat('$y1','-','$month1','-','$d1') && date<=concat('$y2','-','$month2','-','$d2')";
			 $q1 = "SELECT empnum,fname,lname,mname,date,sec_to_time(sum(time_to_sec(timeset))) as TotalHRS FROM tblrecord WHERE date>=concat('$y1','-','$month1','-','$d1') && date<=concat('$y2','-','$month2','-','$d2') GROUP BY lname ASC";
			$q2 = mysql_query($q1);
		}
	}		
	else {
		$q1 = "SELECT * FROM tblrecord WHERE date=' '";
		$q2 = mysql_query($q1);
		
		$r1 = "SELECT * FROM tbluser where empnum=' '";
		$r2 = mysql_query($r1);
	}
...
<body>
	...
	<div id="wrap">
		...
		<div id="content">
			...
			<table style="font-size:14px;width:100%;background:#00FF00;text-align:center">
				<tr>
					<td>Enter range of date to search : e.g. from January 1, 2010 to January 10, 2010</td>
				</tr>
			</table><br>
			...
			<div id='print1'>
				...
				<table width="100%" border="1" style="text-align:center;">
					<tr style="background:#00FF00;font-weight:bold;text-transform:uppercase;">
						<td>#</td>
						<td>Emp#</td>
						<td>Name</td>
						<td>Date</td>
